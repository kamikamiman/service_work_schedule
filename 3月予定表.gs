...
// 読込先スプレットシートを取得する。
const schedule = ssGet.getSheetByName('${period}期3月'.replace('${period}', period));
// 書込先のスプレットシートを取得する。  
const workSchedule = ssSet.getSheetByName('${period}期3月'.replace('${period}', period));
// eサービス、協力会社のシートを取得する。
