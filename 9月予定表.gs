...
function workSchedule_Month9() {
  ...
  // 読込先スプレットシートを取得する。
  const schedule = ssGet.getSheetByName('${period}期9月'.replace('${period}', period));
  ...
  // 書込先のスプレットシートを取得する。  
  const workSchedule = ssSet.getSheetByName('${period}期9月'.replace('${period}', period));
  
  const memberNum = 22; // 人数が増減する場合はこの値を減算
