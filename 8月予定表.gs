...
// 読込先スプレットシートを取得する。
const schedule = ssGet.getSheetByName('${period}期8月'.replace('${period}', period));
// 書込先のスプレットシートを取得する。  
const workSchedule = ssSet.getSheetByName('${period}期8月'.replace('${period}', period));
// eサービス、協力会社のシートを取得する。
...
function HolidayColor () {
  ...
  const firstRowL = 22;     // 開始行(下旬)  * 1 人数増減により変動 * ここを変更します
  const lastRowU  = 20;     // 終了行(上旬)  * 1 人数増減により変動 * ここを変更します
  const lastRowL  = 41;     // 終了行(下旬)  * 2 人数増減により変動 * ここを変更します
